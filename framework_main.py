...
# psychological data position(s) in the inputs. must be integer indicating position (starting from 0)
QI_POS = [1, 2, 3]
...
def generalize(qi, data):
	...
	def review_overlap(f):
		...
		elif (len(overlap) == 1 and f == 1) or len(overlap) == 2:
			return False
...
def _apply_EC_change():
	...
	# after extending, check if other QI entries in accumulated tuples match the new extended EC
	...
	if EC_alter_log:
		for qi in EC_alter_log:
			for tuples in Accumulated_list:
				# QI_EC_indicator = tuples[2][qi]
				# if QI_EC_indicator[qi] equals to original_ec_number, replace it with the new_ec_number
				if tuples[2][qi][qi] == EC_alter_log[qi][0]:
					tuples[2][qi][qi] = EC_alter_log[qi][1]
	...
	try:
		...
		with open("input.txt") as f:
			...
			for sensor_tuple in f:
				...
				# TODO: check data type/amount in list 
				#TODO: check list len > QI_POS
				# strip whitespaces (if read as string)
				#tup = [x.strip() for x in tup]


				with open("output_time.txt", "a") as fo:
					fo.write( str(time.time()) + '\n' )

				# process incoming tuple
				process(tuple_counter, tup)

				print("finish reading line ", sensor_tuple)
